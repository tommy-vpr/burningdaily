...
const FlavorCard = ({
  ...
  return (
    <div className="flex justify-center items-center overflow-hidden cursor-pointer rounded-[5px] relative group w-full h-48">
    <Link href={url}>
        ...
        <h3 className='sm:hidden absolute bottom-4 left-4 text-2xl sm:text-xl text-white font-bold drop-shadow-md'>
            {title}</h3>
        <img className='group-hover:scale-105 transition duration-200 object-cover object-center w-full h-full' alt={title} src={image}/>
    </Link>
</div>
